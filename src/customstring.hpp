...
class CustomString {
 ...
 public:
  ...
  /**
   * @brief operator +
   * @param cs a const CustomString&
   * @return CustomString object
   */
  CustomString operator+(const CustomString& cs);
  /**
   * @brief operator <<
   * @param os output stream
   * @param cs CusotmString
 ...
 private:
   ...
   */
  std::string::const_iterator m_srearchString(
      const std::string& ref, const std::string& str,
...
inline std::string CustomString::data() const { return m_data; }

std::string CustomString::toLowerCase(const std::string& str) {
  std::string tmp;
  transform(str.begin(), str.end(), std::back_inserter<std::string>(tmp),
...
std::string CustomString::toLowerCase() {
  std::string tmp;
  transform(m_data.begin(), m_data.end(), std::back_inserter<std::string>(tmp),
            [](unsigned char c) { return std::tolower(c); });
...
void CustomString::replace(const std::string& oldStr,
...
void CustomString::replaceAll(const std::string& oldStr,
                              const std::string& newStr) {
  size_t pos = 0;
...
void CustomString::replaceIgnCase(const std::string& oldStr,
                                  const std::string& newStr) {
...
void CustomString::replaceAllIgnCase(const std::string& oldStr,
                                     const std::string& newStr) {
  size_t pos = 0;
  while (pos != m_data.length() - 1) {
...
size_t CustomString::oldFindIgnorCase(const std::string& str) {
  std::string::const_iterator it =
      m_srearchString(m_data, str, m_equal_iggnore_case);
  if (it != m_data.end())
    return std::distance(m_data.cbegin(), it);
  else
    return std::string::npos;
...
size_t CustomString::findBoyer(const std::string& str) {
  auto it =
      std::search(m_data.begin(), m_data.end(),
...
std::vector<std::string> CustomString::findPattern(const std::regex& pattern) {
  ...
  while (regex_search(tmp, matcher, pattern)) {
    for (auto found : matcher) founds.push_back(found);
...
inline void CustomString::append(const std::string& str) { m_data += str; }
...
inline CustomString CustomString::operator+(const CustomString& cs) {
  return CustomString(this->m_data + cs.data());
}
